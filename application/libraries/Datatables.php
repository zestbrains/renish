...
class Datatables {
	...
	public function add_column($column, $content, $match_replacement = NULL) {
		$col = trim(preg_replace('/(.*)\s+as\s+(\w*)/i', '$2', $column));
		$this->custCols[] = $col;
	...
	private function get_ordering() {
		...
		$mColArray = $this->custCols;
		$mColArray = array_values(array_diff($mColArray, $this->unset_columns));
		$columns = array_values(array_diff($this->custCols, $this->unset_columns));
	...
	private function get_ordering_bkp() {
			...
			foreach ($custom_val['replacement'] as $key => $val) {
				...
				if ((isset($val)) && $val == "eStatus") {
					$STATUS_BTN = array(
						...
						'class' => 'btn btn-small' . (($replace_string == 'Active') ? ' btn-success' : ' '),
					);
					$custom_val['content'] = str_replace(str_ireplace('$' . ($key + 1), $replace_string, $custom_val['content']), anchor($this->ci->uri->segment(1) . "/status/$2/y", form_button($STATUS_BTN)), str_ireplace('$' . ($key + 1), $replace_string, $custom_val['content']));
				} else {
