...
jQuery('.details-mainn .detdescc .rreadmoree').click(function(){
	jQuery(this).toggleClass('acti');
    jQuery('.details-mainn .detdescc').toggleClass("acti");
});
});
jQuery(window).scroll(function() {
   if(jQuery(window).scrollTop() + jQuery(window).height() > jQuery(document).height() - 200) {
		jQuery('#create_garage .profile-sidebar .panel.panel-default').addClass('active');
   }else{
	   	jQuery('#create_garage .profile-sidebar .panel.panel-default').removeClass('active');
	   }
});